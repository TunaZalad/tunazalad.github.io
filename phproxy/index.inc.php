...
<head>
    ...
    <title>PHProxy</title>
    <script type="text/javascript" src="<?php echo $GLOBALS[
        "_script_base"
    ]; ?>pjs.js"></script>
...
<body bgcolor="#293E71" onload="document.getElementById(10).focus()">
    <br>
    <a href="http://phpr0xi.sourceforge.net/" style="color: white;margin: 0;padding: 0 0 0 15%;" title="<?php echo "PHProxy v" .
        $GLOBALS["_version"]; ?>"><big>PHProxy</big></a>
    ...
    <div id="container">
        <?php switch ($data["category"]) {
            case "auth": ?>
                <div id="auth">
                    <p>
                        <b>Enter your username and password for "<?php echo htmlspecialchars(
                            $data["realm"]
                        ); ?>" on <?php echo $GLOBALS["_url_parts"]["host"]; ?></b>
                    <form method="post" action="">
                        <input type="hidden" name="<?php echo $GLOBALS["_config"][
                            "basic_auth_var_name"
                        ]; ?>" value="<?php echo base64_encode($data["realm"]); ?>" />
                ...
                switch ($data["group"]) {
                    ...
                    case "resource":
                        ...
                        switch ($data["type"]) {
                            case "file_size":
                                $message =
                                    "The file your are attempting to download is too large.<br />" .
                                    "Maxiumum permissible file size is <b>" .
                                    number_format(
                                        $GLOBALS["_config"]["max_file_size"] / 1048576,
                                        2
                                    ) .
                                    " MB</b><br />" .
                                    "Requested file size is <b>" .
                                    number_format(
                                        $GLOBALS["_content_length"] / 1048576,
                                        2
                                    ) .
                                    " MB</b>";
                                break;
                            case "hotlinking":
                                $message =
                                    "It appears that you are trying to access a resource through this proxy from a remote Website.<br />" .
                                    "For security reasons, please use the form below to do so.";
                                break;
                        }
                        break;
                }
                echo "An error has occured while trying to browse through the proxy. <br />" .
                    $message .
                    "</p></div>";
                break;
        } ?>

        <form method="POST" action="<?php echo $GLOBALS[
            "_script_url"
        ]; ?>" name="PHProxy" onsubmit="return PR0XY_G0();" target="_blank"><br>
            <div id="address_bar">
                <label>
                    Web Address: <input type="text" style="width: 66%;" dir="ltr" name="<?php echo $GLOBALS[
                        "_config"
                    ]["url_var_name"]; ?>" id="10" value="<?php echo isset($GLOBALS["_url"])
                         ? htmlspecialchars($GLOBALS["_url"])
                         : ""; ?>" onfocus="this.select()">

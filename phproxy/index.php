...
if ($Login) {
    ...
    $validated =
        in_array($user, $valid_users) && $pass == $valid_passwords[$user];
    if (!$validated) {
...
$_hosts = ["#^127\.|192\.168\.|10\.|172\.(1[6-9]|2[0-9]|3[01])\.|localhost#i"];
$_hotlink_domains = [];
$_insert = [];
$_iflags = "";
$_system = [
...
$_script_url =
    "http" .
    ((isset($_ENV["HTTPS"]) && $_ENV["HTTPS"] == "on") ||
...
{
    return rawurlencode(rawurlencode($name)) .
        "=" .
...
function set_post_vars($array, $parent_key = null)
{
    ...
    foreach ($array as $key => $value) {
        $key = isset($parent_key)
            ? sprintf("%s[%s]", $parent_key, urlencode($key))
            : urlencode($key);
        if (is_array($value)) {
...
function url_parse($url, &$container)
{
    ...
    if (!empty($temp)) {
        ...
        $temp["base"] .= $temp["dir"];
        $temp["prev_dir"] =
            substr_count($temp["path"], "/") > 1
            ? substr($temp["base"], 0, strrpos($temp["base"], "/") + 1)
            : $temp["base"] . "/";
        $container = $temp;
...
function complete_url($url, $proxify = true)
{
    ...
    if ($sep_pos === false || $sep_pos > 5) {
        switch ($url[0]) {
            case "/":
                $url =
                    substr($url, 0, 2) === "//"
                    ? $GLOBALS["_base"]["scheme"] . ":" . $url
                    : $GLOBALS["_base"]["scheme"] .
                    "://" .
                    $GLOBALS["_base"]["host"] .
                    $GLOBALS["_base"]["port_ext"] .
                    $url;
                break;
            case "?":
                $url =
    ...
    return $proxify
        ? "{$GLOBALS["_script_url"]}?{$GLOBALS["_config"]["url_var_name"]}=" .
        encode_url($url) .
        $fragment
        : $url;
}
...
function proxify_inline_css($css)
{
    ...
    for ($i = 0, $count = count($matches); $i < $count; ++$i) {
        $css = str_replace(
            $matches[$i][0],
            "url(" . proxify_css_url($matches[$i][1]) . ")",
            $css
        );
    }
...
function proxify_css($css)
{
    ...
    for ($i = 0, $count = count($matches); $i < $count; ++$i) {
        ...
        $css = str_replace(
            ...
            "@import " .
            $delim .
            proxify_css_url($matches[$i][1]) .
            $delim .
            (isset($matches[$i][4]) ? $matches[$i][4] : ""),
            $css
        );
    }
...
function proxify_css_url($url)
{
    ...
    return $delim .
        preg_replace(
            ...
            '\\$1',
            complete_url(
                trim(preg_replace("#\\\(.)#", '$1', trim($url, $delim)))
            )
        ) .
        $delim;
}
...
function decode_url($url)
{
    return str_replace(
        ["&amp;", "&#38;"],
        "&",
        base64_decode(R0T13(rawurldecode($url)))
    );
}
...
if (
    isset($_POST[$_config["url_var_name"]]) &&
...
if (
    ...
    $_POST[$_config["basic_auth_var_name"]],
    $_POST["username"],
    $_POST["password"]
)
) {
    ...
    $_basic_auth_header = base64_encode(
...
do {
    $_retry = false;
    $_socket = @fsockopen(
        ($_url_parts["scheme"] === "https" && $_system["ssl"]
            ? "ssl://"
            : "tcp://") . $_url_parts["host"],
        $_url_parts["port"],
        $err_no,
    ...
    if (!empty($__url) && strstr($__url, "isP0ST") == true) {
    ...
    if (isset($_url_parts["query"])) {
        ...
        for (
            ...
            implode(
                ...
                array_map(
                    "urlencode",
                    array_map("urldecode", explode("=", $query[$i]))
                )
    ...
    $_request_headers .=
        "Host: " . $_url_parts["host"] . $_url_parts["port_ext"] . "\r\n";
    ...
    if (
        ...
        if (strstr(decode_url($matches[1]), "isP0ST") == true) {
            $_request_headers .=
                "Referer: " .
                strstr(decode_url($matches[1]), "?isP0ST", true) .
                "\r\n";
        } else {
    ...
    if (!empty($_COOKIE)) {
        ...
        foreach ($_COOKIE as $cookie_id => $cookie_content) {
            ...
            if ($cookie_id[0] === "COOKIE") {
                ...
                if (
                    count($cookie_id) < 4 ||
                    ($cookie_content[1] == "secure" &&
                        $_url_parts["scheme"] != "https")
                ) {
                    continue;
                }
                ...
                if (
                    (preg_match(
                        ...
                        strtolower($cookie_id[3]) ==
                        strtolower("." . $_url_parts["host"])) &&
                    preg_match(
                        "#^\Q" . $cookie_id[2] . "\E#",
                        $_url_parts["path"]
                    )
    ...
    if (!empty($_basic_auth_header)) {
        $_set_cookie[] = add_cookie(
            "AUTH;{$_basic_auth_realm};{$_url_parts["host"]}:{$_url_parts["port"]}",
            $_basic_auth_header
        );
        $_request_headers .= "Authorization: Basic {$_basic_auth_header}\r\n";
    } elseif (
        !empty($_basic_auth_realm) &&
        isset($_auth_creds[$_basic_auth_realm])
    ) {
        $_request_headers .= "Authorization: Basic {$_auth_creds[$_basic_auth_realm]}\r\n";
    } elseif (
            list($_basic_auth_realm, $_basic_auth_header) = each($_auth_creds)
    ) {
        $_request_headers .= "Authorization: Basic {$_basic_auth_header}\r\n";
    }
    ...
    if ($_request_method == "POST") {
        if (!empty($_FILES) && $_system["uploads"]) {
            ...
            $_request_headers .=
                "Content-Length: " . strlen($_post_body) . "\r\n\r\n";
    ...
    if (isset($_response_headers["content-type"])) {
        list($_content_type) = explode(
            ";",
            str_replace(
                " ",
                "",
                strtolower($_response_headers["content-type"][0])
            ),
            2
        );
    }
    ...
    if (isset($_response_headers["content-disposition"])) {
        ...
        unset(
            ...
            $_response_keys["content-disposition"]
        );
    }
    if (isset($_response_headers["set-cookie"])) {
        foreach ($_response_headers["set-cookie"] as $cookie) {
            ...
            preg_match("#^\s*([^=;,\s]*)\s*=?\s*([^;]*)#", $cookie, $match) &&
                (list(, $name, $value) = $match);
            preg_match("#;\s*expires\s*=\s*([^;]*)#i", $cookie, $match) &&
                (list(, $expires) = $match);
            preg_match("#;\s*path\s*=\s*([^;,\s]*)#i", $cookie, $match) &&
                (list(, $path) = $match);
            preg_match("#;\s*domain\s*=\s*([^;,\s]*)#i", $cookie, $match) &&
                (list(, $domain) = $match);
            preg_match("#;\s*(secure\b)#i", $cookie, $match) &&
                (list(, $secure) = $match);

            $expires_time = empty($expires) ? 0 : intval(@strtotime($expires));
            $expires =
                !empty($expires) && time() - $expires_time < 0 ? "" : $expires;
            ...
            if (empty($domain)) {
                ...
                $domain =
                    "." .
                    strtolower(str_replace("..", ".", trim($domain, ".")));
                ...
                if (
                    (!preg_match(
                        ...
                        $domain != "." . $_url_parts["host"]) ||
                    (substr_count($domain, ".") < 2 && $domain[0] == ".")
                ) {
                    continue;
                }
            }
    ...
    if (
        isset($_response_headers["p3p"]) &&
        preg_match(
            '#policyref\s*=\s*[\'"]?([^\'"\s]*)[\'"]?#i',
            $_response_headers["p3p"][0],
            $matches
        )
    ) {
        $_response_headers["p3p"][0] = str_replace(
            $matches[0],
            'policyref="' . complete_url($matches[1]) . '"',
            $_response_headers["p3p"][0]
        );
    }
    if (
        isset($_response_headers["refresh"]) &&
        preg_match(
            "#([0-9\s]*;\s*URL\s*=)\s*(\S*)#i",
            $_response_headers["refresh"][0],
            $matches
        )
    ) {
        $_response_headers["refresh"][0] =
            $matches[1] . complete_url($matches[2]);
    }
    if (isset($_response_headers["location"])) {
        $_response_headers["location"][0] = complete_url(
            $_response_headers["location"][0]
        );
    }
    if (isset($_response_headers["uri"])) {
        $_response_headers["uri"][0] = complete_url(
            $_response_headers["uri"][0]
        );
    }
    if (isset($_response_headers["content-location"])) {
        $_response_headers["content-location"][0] = complete_url(
            $_response_headers["content-location"][0]
        );
    }
    ...
    if (
        $_response_code == 401 &&
        isset($_response_headers["www-authenticate"]) &&
        preg_match(
            '#basic\s+(?:realm="(.*?)")?#i',
            $_response_headers["www-authenticate"][0],
            $matches
        )
    ) {
...
if (!isset($_proxify[$_content_type])) {
    ...
    if ($_content_length !== false) {
        if (
            ...
            $_content_length > $_config["max_file_size"]
        ) {
            show_report([
                "which" => "index",
                "category" => "error",
...
if ($_content_type == "text/css") {
    ...
    for ($i = 0, $count_i = count($matches); $i < $count_i; ++$i) {
        $_response_body = str_replace(
            $matches[$i][0],
            $matches[$i][1] . proxify_css($matches[$i][2]) . $matches[$i][3],
            $_response_body
        );
    }
    ...
    for ($i = 0, $count_i = count($matches[0]); $i < $count_i; ++$i) {
        ...
        for (
            $j = 0, $count_j = count($m);
            $j < $count_j;
            $attrs[strtolower($m[$j][1])] = isset($m[$j][4])
            ? $m[$j][4]
            : (isset($m[$j][3])
                ? $m[$j][3]
                : (isset($m[$j][2])
            ...
            switch ($tag) {
                ...
                case "form":
                    ...
                    if (
                        !isset($attrs["method"]) ||
                        trim($attrs["method"]) === "" ||
                        strtolower(trim($attrs["method"])) === "get"
                    ) {
                        $attrs["method"] = "POST";
                        $extra_html =
                            '<input type="hidden" name="P0ST2GET" value="1">';
                    }
                ...
                case "meta":
                    if (
                        isset($attrs["http-equiv"], $attrs["content"]) &&
                        preg_match("#\s*refresh\s*#i", $attrs["http-equiv"])
                    ) {
                        if (
                            preg_match(
                                "#^(\s*[0-9]*\s*;\s*url=)(.*)#i",
                ...
                case "head":
                    if (isset($attrs["profile"])) {
                        $rebuild = true;
                        $attrs["profile"] = implode(
                            " ",
                            array_map(
                                "complete_url",
                                explode(" ", $attrs["profile"])
                            )
                        );
                    }
                    break;
                ...
                case "object":
                    ...
                    if (
                        isset($attrs["classid"]) &&
                        !preg_match("#^clsid:#i", $attrs["classid"])
                    ) {
                        $rebuild = true;
                ...
                case "param":
                    if (
                        isset($attrs["valuetype"], $attrs["value"]) &&
                        strtolower($attrs["valuetype"]) == "ref" &&
                        preg_match("#^[\w.+-]+://#", $attrs["value"])
                    ) {
                        $rebuild = true;
    ...
    if (!isset($_GET["nf"])) {
        $_url_form =
            ...
            $GLOBALS["_script_url"] .
            '" name="PHProxy" class="pmfclass" target="_top" onsubmit="return PR0XY_G0();">Address :' .
            ' [<input class="pmfclass" type="text" style="width: 68%;" name="' .
            $GLOBALS["_config"]["url_var_name"] .
            '" value="' .
            rawurldecode($_url) .
            (!empty($POST) ? urldecode("?isP0ST=1&" . $POST) : "") .
            '">] ' .
            ...
            'document.writeln(\' <a id="pmfhref" class="pmfclass" style="color: #FF0000;cursor: pointer;" onclick="document.getElementById (110011).style.display = ' .
            "\'none\'" .
            '" title="Hide mini form"> &nbsp; &nbsp; X</a>\');' .
            'document.PHProxy.method = \'GET\';' .
            "</script>" .
            "</form></div>";

        $_response_body = preg_replace(
            "#\<\s*body(.*?)\>#si",
            "$0\n$_url_form",
            $_response_body,
            1
...
$_response_keys["content-disposition"] = "Content-Disposition";
$_response_headers["content-disposition"][0] = empty($_content_disp)
    ? ($_content_type == "application/octet_stream" ? "attachment" : "inline") .
    '; filename="' .
    $_url_parts["file"] .
    '"'
    : $_content_disp;
